...
on("chat:message", function(msg) {
	...
	if (msg.type == "api" && msg.content.indexOf("!tesoro5e ") !== -1) {
		// !tesoro5e personal/acumulado CR
		msg.content = msg.content.replace("!tesoro5e ", "");
		
		let params = msg.content.split(" ");
		let acumulado = false;
		if (params.length < 2) {
			log("ERROR !tesoro5e main: faltan parámetros");
			return;
		}
		...
		if (params[0] == "acumulado") {
			...
			log("ERROR !tesoro5e main: parámetro acumulado/individual incorrecto");
			return;
		}
		...
		if (isNaN(desafio)) {
			log("ERROR !tesoro5e main: parámetro desafío incorrecto");
			return;
		}
		...
		let chorrazo = "/w GM &{template:default}{{name=Tesoro " + params[0] + " desafío " + params[1] + "}}";
