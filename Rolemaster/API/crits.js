let tablaAplastamiento = {
	//roll: 0;
	indices: [5,20,35,50,65,79,80,86,89,90,96,99,100,106,109,110,116,119,9999999],
	resultados: [
	...
	getCritico(roll) {
		return this.resultados[this.indices.findIndex(function(value) {
			return value >= roll;
		})];
	}		
};
...
//tablaAplastamiento.roll = 90;
// log(tablaAplastamiento.getCritico(90));
...
on("chat:message", function(msg) {
  if(msg.type == "api" && msg.content.indexOf("!crit ") !== -1) {
    ...
    msg.content = msg.content.replace("!crit ", "");
    let modificador = 0;
	let tabla = null;
	let gravedad = "";
	let tipoCritico = "";
	let indice = 0;
	if (msg.content[0] == "T") modificador = -50;
	else if (msg.content[0] == "A") modificador = -20;
	else if (msg.content[0] == "B") modificador = -10;
	else if (msg.content[0] == "C") modificador = 0;
	else if (msg.content[0] == "D") modificador = 10;
	else if (msg.content[0] == "E") modificador = 20;
	else if (msg.content[0] == "N") return;
	else {
  		sendChat(msg.who, "Error de sintaxis. Formato: !crit EK100");
		return;
	}
	gravedad = msg.content[0];
	if (msg.content[1] == "K") {
		tabla = tablaAplastamiento;
		tipoCritico = "Aplastamiento";} 
	else if (msg.content[1] == "S") {
		tabla = tablaTajo;
		tipoCritico = "Tajo";} 
	else if (msg.content[1] == "P") {
		tabla = tablaPerforacion;
		tipoCritico = "Perforación";} 
	else if (msg.content[1] == "U") {
		tabla = tablaDesequilibrio;
		tipoCritico = "Desequilibrio";} 
	else if (msg.content[1] == "G") {
		tabla = tablaPresa;
		tipoCritico = "Presa";} 
	...
	tirada = parseInt(msg.content.substring(2),10);
	...
	// let resultado = "&{template:default}{{name=Crítico " + gravedad + " (" + strModificador + ") de " + tipoCritico + "}}{{Tirada[" + indice + "]="+tabla.getCritico(indice)+"}}";
	let resultado = "/w GM &{template:critico}{{gravedad=" + gravedad + "}}{{tipoCritico= " + tipoCritico + "}}{{tirada="+tirada+"}}{{resultado="+tabla.getCritico(indice)+"}}";
	//resultado = tabla.getCritico(indice);
	// log(resultado);
	sendChat(msg.who, resultado);
	} else if(msg.type == "api" && msg.content.indexOf("!pif ") !== -1) {
	    if(_.has(msg,'inlinerolls')){
            msg.content = _.chain(msg.inlinerolls)
                .reduce(function(m,v,k){
                    m['$[['+k+']]']=v.results.total || 0;
	...
	msg.content = msg.content.replace("!pif ", "");
	let tipoPifia = "";
	let tabla = null;
	if (msg.content[0] == "E")      {tipoPifia="armas empuñadas";tabla=pifiasEmpuñadas;}
	else if (msg.content[0] == "P") {tipoPifia="armas de proyectil";tabla=pifiasProyectil;}
	else if (msg.content[0] == "S") {tipoPifia="sortilegios";tabla=pifiasSortilegios;}
	else if (msg.content[0] == "M") {tipoPifia="maniobras de movimiento";tabla=pifiasManiobrasMovimiento;}
	let modificador = parseInt(msg.content.substring(1,4));
	let tirada = parseInt(msg.content.substring(4));
	let indice = tirada + modificador;
	let strModificador = "";
	if (modificador<0) strModificador = modificador;
	else strModificador = "+"+modificador;
	let resultado = "/w GM &{template:pifia}{{tipoPifia= " + tipoPifia + "}}{{tirada="+tirada+"}}{{modificador="+strModificador+"}}{{tirada modificada="+indice+"}}{{resultado="+tabla.getCritico(indice)+"}}";
	sendChat(msg.who, resultado);
  }
});