let tablaManiobrasEstaticasGeneral = {
	//roll: 0;
	indices: [-26,04,75,90,110,175,9999999],
	resultados: [
...
on("chat:message", function(msg) {
  if(msg.type == "api" && msg.content.indexOf("!mest ") !== -1) {
    ...
    msg.content = msg.content.replace("!mest ", "");
    log (msg.content);
    let pos = msg.content.indexOf(";");
    let tirada = parseInt(msg.content.substring(1,pos),10);
	 let tabla = null;
	 let modificador = parseInt(msg.content.substring(pos+1),10);
	let tipoManiobra = "";
	 let indice = 0;
	if (msg.content[0] == "G") {
		tabla = tablaManiobrasEstaticasGeneral;
		tipoManiobra = "General";} 
	else if (msg.content[0] == "I") {
		tabla = tablaManiobrasEstaticasInfluenciaInteraccion;
		tipoManiobra = "Influencia e Interacción";} 
	else if (msg.content[0] == "L") {
	...
	let resultado = "/w GM &{template:maniobraestatica}{{tipoManiobra= " + tipoManiobra + "}}{{tirada="+tirada+"\uD83C\uDFB2 "+strModificador+" = "+indice+"}}{{resultado="+tabla.getResultado(indice)+"}}";
